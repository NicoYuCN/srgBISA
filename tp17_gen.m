function mapstage1 = tp17_gen(rootPath,imgPath,imname,imgtype)
...
saldir=[rootPath,'saliencymap/'];% the output path of the saliency map
supdir=[rootPath,'superpixels/'];% the superpixel label file path
if ~exist(supdir,'dir')
    mkdir(supdir);
    mkdir(saldir);
end

imnames=[imgPath imname imgtype];
[input_im, w]=removeframe(rootPath,imgPath,imname,imgtype);% run a pre-processing to remove the image frame 
...
outname = [rootPath,imname,'.bmp'];
%system('open matlab.exe --args -r ')
comm=['SLICSuperpixelSegmentation' ' ' outname ' ' int2str(20) ' ' int2str(spnumber) ' ' supdir];
system(comm);   
spname=[supdir imname  '.dat'];
...
outname=[saldir imname imgtype];
imwrite(mapstage1,outname);

clear  outname w salall m n
end
